...
var DbSelectorValue = (function () {
    ...
    function DbSelectorValue(selector, propertyContainer, propertyName) {
        ...
        assert_1.Assert.isTruthy(propertyContainer, 'DbSelectorValue: Invalid parent object');
        assert_1.Assert.isTruthy(propertyName, 'DbSelectorValue: Invalid property name');
        assert_1.Assert.isTruthy(selector, 'DbSelectorValue: Invalid sector');
    }
...
var DbSelector = (function () {
    ...
    DbSelector.createWithProperty = function (field, item) {
        var selector = new DbSelector();
        return selector.withProperty(utilities_1.Utilities.getPropertyName(field)).withValue(item).on();
    };
    DbSelector.createWithObjectValue = function (field, item) {
        if (item === void 0) { item = {}; }
        var selector = new DbSelector();
        return selector.withProperty(utilities_1.Utilities.getPropertyName(field)).withObjectValue(item).on();
    };
    ...
    DbSelector.prototype.withProperty = function (field) {
        assert_1.Assert.isTruthy(field, 'DbSelector: Property field cannot be null/empty');
        var name = utilities_1.Utilities.getPropertyName(field);
...
function applyWildCardToString(data) {
    return data + "\uFFF0";
}
exports.applyWildCardToString = applyWildCardToString;
